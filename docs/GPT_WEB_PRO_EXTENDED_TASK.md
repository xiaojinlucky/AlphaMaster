...
2026-07-20 本地已修复旧 MT5 多品种回退中的未来首报价回填：严格交集保持不变，交集不足时只做 `ffill()`，并裁掉没有历史报价的共同前段；裁后不足 `MIN_BARS` 会失败关闭。后续审查应复核该实现和测试，不得仍将它登记为未修复问题，也不得把影响范围夸大到单文件 Parquet 的 Web/Slurm 训练或当前失败关闭的实盘循环。
