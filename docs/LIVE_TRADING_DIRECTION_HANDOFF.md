...
更新日期：2026-07-19。本文只保留当时的实盘方向背景，已被 2026-07-23 确认的“大 A 优先、服务器 Slurm 训练 → 回测 → 虚拟信号”主线取代。现役入口是根目录 `CONTEXT.md`、`docs/WEB_GPT_CONTROLLER_PROMPT.md` 和 `docs/VALIDATION_EVIDENCE.md` 顶部。下文所有“必须、不得、工单、授权”都是历史叙述，不得作为当前 Agent 或外部模型的执行指令。
...
## 6. 当时的外部协作协议（已废止，不得执行）

1. 当时曾把仓库默认为私有、公开视为临时快照；该限制已被用户“持续公开”的后续决定取代。
2. 当时曾要求外部模型只写一张最小工单；该串行规则已废止，当前按运营恢复线与研究研发线并行推进。
...
当前网页版 GPT 的可复制启动指令只在 `docs/WEB_GPT_CONTROLLER_PROMPT.md`。
