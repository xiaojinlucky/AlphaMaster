...
这些问题会影响训练评分、冠军选择和特征评估。当前实验只复现并冻结证据，
没有修改生产评分代码；正在运行的旧源码 Slurm 批次也没有被取消、重提或
覆盖。
...
这同时证明额外右移和补零污染。其余两处 P0（PnL 尾部裁剪、模块级
`score_all`/`EffectivenessEvaluator` 的默认 horizon）来自当前生产源码
审查，尚未用本轮运行探针端到端执行。
...
1. 在新源码身份下同时修复两处 IC 额外右移、PnL 尾部补零污染和
   `score_all`/`EffectivenessEvaluator` 的 horizon，再补 PnL、IC、
   标签、成交统一时钟回归。
2. 实现 AlphaMaster 自有历史成分区间合同和真实历史 A50 数据入口。
3. 补历史时点 ST、板块、涨跌停、停牌和成交量状态。
4. 再做跨日挂单、公司行动和按日期生效的费用政策回放。
