...
本节只记录 Slurm 第一阶段实施当时的范围，不是现行前端禁令。当时不新增页面、路由、导航、布局、组件层级、角色可见性或视觉风格；现有“开始/停止/日志/进度”交互保持不变，后端切换为 Slurm 并返回兼容状态，因此该阶段没有新的前端设计任务。后续三页视觉统一已经单独使用 Google Stitch 形成参考并在提交 `186217e` 落地。
