...
> 日期：2026-07-24
> 状态：第一阶段完成；小红书、抖音完整正文与评论区仍待登录补证，E3/E4 尚未执行
...
1. **保留 AlphaMaster 的核心执行与审计链。**
   当前 `portfolio_manager/execution.py` 的源码和单元测试已有 T+1、显式停牌/涨跌停状态、整手、合法碎股清仓、最低佣金、印花税、过户费、滑点和确定性执行身份，但尚未接入现役 Web 长驻链路。截至本轮 E0—E2 证据，尚未证明 AKQuant、RQAlpha、Qlib、vn.py 中任何一个完整覆盖这组能力；已确认的模块级缺口见第三节。
2. **优先复用成熟的非核心轮子。**
...
本轮把核心候选推进到 E2；长尾候选分别停在 E0—E2，等级见各项结论。E3/E4 是下一阶段，不把源码阅读说成已经集成验证。
...
|---|---|---|---|---|
| [AKQuant](https://github.com/akfamily/akquant) | E2 | 高优先级隔离复验 | T+1、订单状态、黄金基线和恢复测试 | 项目很新；[#329](https://github.com/akfamily/akquant/issues/329)、[#334](https://github.com/akfamily/akquant/issues/334) 是同一报告者发现、现已关闭并有回归测试的历史缺陷；历史 ST/板块规则仍需外部时点数据 |
| [RQAlpha](https://github.com/ricequant/rqalpha) | E2 | A 股语义参考与对照器 | 可卖持仓、碎股清仓、公司行动、退市、时点印花税、分笔最低佣金 | 高质量历史数据依赖 RQData；本轮审计的默认股票费用决策器未计 AlphaMaster 已有的过户费；自定义非商业许可 |
...
| [vn.py alpha](https://github.com/vnpy/vnpy/tree/master/vnpy/alpha) | E2 | 高优先级研究层复验 | Polars 因子处理器、Alpha101/158、统一模型接口、按日成分列表 | 本轮固定快照的 alpha 回测器存在用合成旧价 bar 撮合的路径，且该撮合器没有独立现金校验；不能泛化为整个 vn.py |
...
| 漏洞 | 当前影响 | 应吸收的轮子 |
|---|---|---|
| 当前 A50 是 2026 冻结成分，没有历史生效区间 | 组合级历史回测会有幸存者偏差；当前单股工程基线不得冒充历史指数策略 | Qlib 的成分生效区间；vn.py alpha 的“日期 → 成分列表”及其派生区间 |
| 缺历史时点 ST、板块、涨跌停、停牌和成交量状态 | 执行器虽然正确，但上游无法给出完整历史 `ExecutionQuote` | AKQuant 的显式规则字段、tickflow 的日期化涨跌停测试 |
| 信号、IC、标签和 PnL 时钟仍有待合成验证 | 当前标签合同是 `target_ret[t] = log(open[t+2] / open[t+1])`；需验证 `factor[t]`、t+1 开盘成交和 t+1→t+2 持有收益完全一致 | Qlib/vn.py 的前一周期信号与下一根成交结构 |
| 训练循环反复使用 validation 指导冠军、精英池和搜索方向 | validation 已是选择集，不能称最终样本外 | skfolio 的时间序列评估思想；最终仍由 AlphaMaster 密封合同控制 |
| 连续仓位单费率回测与 A 股虚拟执行是两套语义 | 同数据 replay 可能高估可成交性，不能和最终 A 股结果混称 | AKQuant/RQAlpha 只作为逐订单对照 |
...
3. 在 AlphaMaster 隔离环境执行 AKQuant/RQAlpha/Qlib/vn.py 的 E3/E4 差分。
...
|---|---|---|---|---|---|
| AKQuant 固定快照、#329、#334 | 2026-07-24 | 源码/测试/Issue，E2 | 两个 Issue 已关闭 | A 股状态与回归测试值得复用 | 同一报告者；历史缺陷不是当前故障 |
| RQAlpha 固定快照 | 2026-07-24 | 源码，E2 | 固定提交 | 可卖持仓、公司行动、默认费用语义 | RQData 和自定义许可限制；尚未 E3 |
| Qlib 固定快照、#1875、#1930 | 2026-07-24 | 源码/Issue，E2 | #1875 对应问题已修；#1930 为用户复现 | 研究层强，实验身份和采集失败要审计 | Issue 不代表全部版本；尚未 E3 |
| vn.py alpha 固定快照 | 2026-07-24 | 源码，E2 | 固定提交 | 因子/模型接口值得借，alpha 撮合不作 A 股真值 | 结论只适用于 alpha 模块 |
