...
- 当前公式兼容版本为 `vb6aac3246b27`；仓库内 `v9217a2c0d91a` 历史策略仅保留作旧证据，不能续训、回测或实时使用，后续必须基于新语义重新训练，不做静默迁移。
...
经用户确认后按精确文件范围提交并推送因果 normalization 工单。旧策略已经失效；正式训练预算仍未确定，本轮不得为了生成新策略擅自启动长训练。旧 MT5 批量计划也仍未获 apply 授权。
